...
# cmd コマンドコード
# delay 待機秒数
def command(cmd:lcd_cmd ,delay_us:float):
...
def init ():
        #初期化
        command(lcd_cmd.lcd_function_set + lcd_cmd_param.lcd_function_set_data_width + lcd_cmd_param.lcd_function_set_lines)
        #拡張コマンド
        command(lcd_cmd.lcd_function_set + lcd_cmd_param.lcd_function_set_data_width + lcd_cmd_param.lcd_function_set_lines + lcd_cmd_param.lcd_function_set_ext)
        #周波数セット
        command(lcd_cmd.lcd_ext_osc_freq + lcd_cmd_param.lcd_ext_osc_freq_normal)
        #コントラスト下位
        command(lcd_cmd.lcd_ext_contrast + lcd_cmd_param.lcd_ext_contrast_normal)
        #コントラスト上位 
        command(lcd_cmd.lcd_ext_pwiccn + lcd_cmd_param.lcd_ext_pwiccn_boost + lcd_cmd_param.lcd_ext_pwiccn_contrast_3V)
        #増幅率
        command(lcd_cmd.lcd_ext_follower + lcd_cmd_param.lcd_ext_follower_on + lcd_cmd_param.lcd_ext_follower_pow_normal)
        #基本コマンド
        command(lcd_cmd.lcd_function_set + lcd_cmd_param.lcd_function_set_data_width + lcd_cmd_param.lcd_function_set_lines)
        #ディスプレイオン
        command(lcd_cmd.lcd_display + lcd_cmd_param.lcd_display_on)
        #クリア
        command(lcd_cmd.lcd_clear)
...
while 1:
        ...
        command(lcd_cmd.lcd_set_ddram_address+lcd_cmd_param.lcd_set_ddram_address_line1)
        writeMessage(line1);
        command(lcd_cmd.lcd_set_ddram_address+lcd_cmd_param.lcd_set_ddram_address_line2)
        writeMessage(line2)
